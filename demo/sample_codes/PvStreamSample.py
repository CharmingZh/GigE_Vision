"""
文件名称: PvStreamSample.py
功能描述:
    该脚本演示了如何使用 `PvStream` 对象从 GigE Vision 或 USB3 Vision 设备采集图像。
    与 `PvPipeline` 不同，`PvStream` 需要手动管理缓冲区（分配、排队、检索、释放）。
    这提供了更底层的控制，但也需要更多的代码来处理缓冲区生命周期。

特别注意事项:
    1. 脚本依赖 `opencv-python` (cv2) 进行图像显示。
    2. 演示了手动创建和管理 `PvBuffer` 列表。
    3. 展示了如何处理 `PvStream` 的缓冲区队列（QueueBuffer, RetrieveBuffer）。
"""
#!/usr/bin/env python3
'''
*****************************************************************************
...
def configure_stream_buffers(device, stream):
    ...
    # 使用 BUFFER_COUNT 或最大缓冲区数量，以较小者为准
